...
## Project Structure
- `src/`: root `justjavac/lepus` facade plus Lepus subpackages.
- `src/webview/`: `justjavac/lepus/webview`; low-level CEF-backed native binding.
- `src/manifest/`: `justjavac/lepus/manifest`; owns `app.json`-style manifest types and declarative extension settings.
- `src/core/`: `justjavac/lepus/core`; owns ops dispatch, JS bridge installation, extension host, and `window.__MoonBit__`.
...
## Architectural Rules
- Keep dependencies acyclic and flowing downward: `webview <- core <- runtime <- lepus`, `manifest <- bootstrap <- lepus`, `ipc <- core/runtime/lepus`, `core <- lepus_ext`.
...
- The root `lepus` facade should stay focused on ergonomic composition from `manifest + explicitly linked extensions` into a runtime app, with a small and clear public API.
