...
static proton_browser_policy_mode_t proton_browser_policy_default(
    const char *field) {
  return strcmp(field, "navigation") == 0 ? PROTON_BROWSER_POLICY_ALLOW
                                           : PROTON_BROWSER_POLICY_DENY;
}

static int proton_browser_read_policy_mode(
    const proton_json_doc_t *doc, proton_json_value_t object,
    const char *field, proton_browser_policy_mode_t *out_mode,
    char *error, size_t error_len) {
  proton_json_value_t value = {0};
  *out_mode = proton_browser_policy_default(field);
  if (!proton_json_object_get(doc, object, field, &value)) {
    return 1;
  }
  char mode[16] = {0};
  if (!proton_json_read_string(doc, value, mode, sizeof(mode))) {
    proton_browser_set_message(error, error_len,
                               "browser policy value must be a string");
    return 0;
  }
  if (strcmp(mode, "allow") == 0) {
    *out_mode = PROTON_BROWSER_POLICY_ALLOW;
  } else if (strcmp(mode, "deny") == 0) {
    *out_mode = PROTON_BROWSER_POLICY_DENY;
  } else if (strcmp(mode, "ask") == 0) {
    *out_mode = PROTON_BROWSER_POLICY_ASK;
  } else {
    proton_browser_set_message(
        error, error_len,
        "browser policy must be \"allow\", \"deny\", or \"ask\"");
    return 0;
  }
  return 1;
}
...
int32_t proton_browser_policy_parse_window_json(
    ...
    char *error, size_t error_len) {
  if (window_json == NULL || out_policy == NULL) {
    proton_browser_set_message(error, error_len,
                               "window JSON and browser policy are required");
    return PROTON_ERR_INVALID_ARGUMENT;
  }
  *out_policy = (proton_browser_policy_t){
      .navigation = PROTON_BROWSER_POLICY_ALLOW,
      .popup = PROTON_BROWSER_POLICY_DENY,
      .download = PROTON_BROWSER_POLICY_DENY,
      .certificate = PROTON_BROWSER_POLICY_DENY,
      .media = PROTON_BROWSER_POLICY_DENY,
      .devtools = 0,
  };
  proton_json_doc_t doc = {0};
  proton_json_value_t root = {0};
  proton_json_value_t browser = {0};
  if (!proton_json_parse(&doc, window_json) ||
      !proton_json_root_object(&doc, &root)) {
    proton_json_dispose(&doc);
    proton_browser_set_message(error, error_len, "window JSON is invalid");
    return PROTON_ERR_INVALID_ARGUMENT;
  }
  if (!proton_json_object_get(&doc, root, "browser", &browser)) {
    proton_json_dispose(&doc);
    return PROTON_OK;
  }
  if (!proton_json_is_object(&doc, browser)) {
    proton_json_dispose(&doc);
    proton_browser_set_message(error, error_len,
                               "window browser policy must be an object");
    return PROTON_ERR_INVALID_ARGUMENT;
  }
  int valid =
      proton_browser_read_policy_mode(&doc, browser, "navigation",
                                      &out_policy->navigation, error, error_len) &&
      proton_browser_read_policy_mode(&doc, browser, "popup",
                                      &out_policy->popup, error, error_len) &&
      proton_browser_read_policy_mode(&doc, browser, "download",
                                      &out_policy->download, error, error_len) &&
      proton_browser_read_policy_mode(&doc, browser, "certificate",
                                      &out_policy->certificate, error,
                                      error_len) &&
      proton_browser_read_policy_mode(&doc, browser, "media",
                                      &out_policy->media, error, error_len);
  proton_json_value_t devtools = {0};
  if (valid && proton_json_object_get(&doc, browser, "devtools", &devtools)) {
    bool enabled = false;
    valid = proton_json_read_bool(&doc, devtools, &enabled);
    out_policy->devtools = enabled ? 1 : 0;
    if (!valid) {
      proton_browser_set_message(error, error_len,
                                 "browser devtools must be a boolean");
    }
  }
  proton_json_dispose(&doc);
  return valid ? PROTON_OK : PROTON_ERR_INVALID_ARGUMENT;
}
